...
impl AstBasedFileSplitter {
    ...
    pub async fn vectorization_split(
        ...
        let (mut parser, language) = match get_ast_parser_by_filename(&path) {
            ...
            Err(_e) => {
                ...
                return self
                    ...
                    .vectorization_split(&doc, tokenizer.clone(), tokens_limit, gcx.clone())
                    .await;
            }
        };
        ...
        let ast_markup: FileASTMarkup =
            match crate::ast::lowlevel_file_markup(&doc, &symbols_struct) {
                Ok(x) => x,
                Err(e) => {
                    tracing::info!(
        ...
        for guid in &guids {
            ...
            let need_in_vecdb_at_all = match symbol.symbol_type {
                ...
                | SymbolType::ClassFieldDeclaration => true,
                _ => false,
            };
