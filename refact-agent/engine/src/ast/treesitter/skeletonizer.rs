...
pub trait SkeletonFormatter {
    ...
    fn preprocess_content(&self, content: Vec<String>) -> Vec<String> {
        ...
        let lines = content
            .iter()
            .map(|x| x.replace("\r", "").replace("\t", "    ").to_string())
            .collect::<Vec<_>>();
        let indent_n = content
            .iter()
            .map(|x| {
                if x.is_empty() {
                    return usize::MAX;
    ...
    fn get_declaration_with_comments(
        ...
        let mut all_top_syms = guid_to_info
            .values()
            .filter(|info| info.full_range.start_point.row < top_row)
            .collect::<Vec<_>>();
        // reverse sort
        all_top_syms.sort_by(|a, b| {
            b.full_range
                .start_point
                .row
                .cmp(&a.full_range.start_point.row)
        });
        ...
        let mut need_syms: Vec<&&SymbolInformation> = vec![];
        {
            for idx in 0..all_top_syms.len() {
                ...
                let all_sym_on_this_line = all_top_syms
                    .iter()
                    .filter(|info| {
                        info.full_range.start_point.row == sym.full_range.start_point.row
                            || info.full_range.end_point.row == sym.full_range.start_point.row
                    })
                    .collect::<Vec<_>>();

                if all_sym_on_this_line
                    .iter()
                    .all(|info| info.symbol_type == SymbolType::CommentDefinition)
                {
                    need_syms.push(sym);
                } else {
                    break;
