...
use crate::ast::treesitter::parsers::tests::{
    base_parser_test, base_skeletonizer_test, base_declaration_formatter_test,
};

#[test]
fn test_kotlin_main() {
    let parser = KotlinParser::new().unwrap();
    let mut boxed_parser: Box<dyn crate::ast::treesitter::parsers::AstLanguageParser> =
        Box::new(parser);
...
"#;
    ...
    for (i, symbol) in symbols.iter().enumerate() {
        ...
        if let Some(prop) = sym
            .as_any()
            .downcast_ref::<crate::ast::treesitter::ast_instance_structs::ClassFieldDeclaration>(
        ) {
