...
pub fn create_robot_human_record_if_not_exists(
    ...
    let record = TeleRobotHumanAccum::new(uri.clone(), text.clone());
...
fn update_robot_characters_baseline(rec: &mut TeleRobotHumanAccum, snip: &SnippetTracker) {
...
fn basetext_to_text_leap_calculations(
    ...
    let re = Regex::new(r"\s+").unwrap();
    let (added_characters, removed_characters) =
        utils::get_add_del_from_texts(&baseline_text, text);
    ...
    if human_characters.max(1) / time_diff_s > MAX_CHARS_PER_SECOND {
        debug!(
            "ignoring human_character: {}; probably copy-paste; time_diff_s: {}",
            human_characters, time_diff_s
        );
        human_characters = 0;
    }
    debug!(
        "human_characters: +{}; robot_characters: +{}",
        0.max(human_characters),
        rec.robot_characters_acc_baseline
    );
...
pub fn increase_counters_from_accepted_snippet(
    ...
    let now = chrono::Local::now().timestamp();
    if let Some(rec) = storage_locked
        .tele_robot_human
        .iter_mut()
        .find(|stat| stat.uri.eq(uri))
    {
...
fn compress_robot_human(storage_locked: &RwLockReadGuard<Storage>) -> Vec<TeleRobotHuman> {
    let mut unique_combinations: HashMap<(String, String), Vec<TeleRobotHumanAccum>> =
        HashMap::new();
    ...
    for (key, entries) in unique_combinations {
        let mut record = TeleRobotHuman::new(key.0.clone(), key.1.clone());
...
pub async fn tele_robot_human_compress_to_file(cx: Arc<ARwLock<global_context::GlobalContext>>) {
    let mut records = vec![];
...
impl TeleRobotHuman {
    fn new(file_extension: String, model: String) -> Self {
        Self {
            file_extension,
            model,
