...
use crate::caps::{
    ChatModelRecord, CompletionModelFamily, CompletionModelRecord, EmbeddingModelRecord,
    HasBaseModelRecord,
};
use crate::custom_error::{MapErrToString, ScratchError};
use crate::global_context::{try_load_caps_quickly_if_not_present, GlobalContext};
use crate::caps::providers::{
    get_known_models, get_provider_from_server, get_provider_from_template_and_config_file,
    get_provider_model_default_settings_ui, get_provider_templates, read_providers_d, CapsProvider,
};
...
impl ProviderDTO {
    pub fn from_caps_provider(provider: CapsProvider, readonly: bool) -> Self {
        ProviderDTO {
            ...
            completion_endpoint: if provider.supports_completion {
...
fn model_type_completion() -> ModelType {
...
fn model_type_embedding() -> ModelType {
    ModelType::Embedding
}
...
pub async fn handle_v1_providers(
    ...
    match crate::global_context::try_load_caps_quickly_if_not_present(gcx.clone(), 0).await {
        ...
        Err(e) => {
            tracing::error!(
                "Failed to load caps, server provider will not be included: {}",
                e
            );
        }
    }
...
pub async fn handle_v1_get_provider(
    ...
    let provider_dto = if use_server_provider {
        let provider = get_provider_from_server(gcx.clone())
            .await
        ...
        let provider = get_provider_from_template_and_config_file(
            &config_dir,
...
pub async fn handle_v1_post_provider(
    ...
    let provider_dto = serde_json::from_slice::<ProviderDTO>(&body_bytes).map_err(|e| {
        ScratchError::new(
            StatusCode::UNPROCESSABLE_ENTITY,
            format!("Error parsing provider: {}", e),
        )
    })?;
    ...
    let provider_path = config_dir
        .join("providers.d")
        .join(format!("{}.yaml", provider_dto.name));

    let provider_template = get_provider_templates()
        .get(&provider_dto.name)
        .cloned()
        .ok_or(ScratchError::new(
            StatusCode::UNPROCESSABLE_ENTITY,
            "Provider template not found".to_string(),
        ))?;

    let mut file_value = read_yaml_file_as_value_if_exists(&provider_path)
        .await
        .map_err(|e| ScratchError::new(StatusCode::INTERNAL_SERVER_ERROR, e))?;

    update_yaml_field_if_needed(
        &mut file_value,
        "endpoint_style",
        provider_dto.endpoint_style,
        provider_template.endpoint_style,
    );
    update_yaml_field_if_needed(
        &mut file_value,
        "api_key",
        provider_dto.api_key,
        provider_template.api_key,
    );
    update_yaml_field_if_needed(
        &mut file_value,
        "tokenizer_api_key",
        provider_dto.tokenizer_api_key,
        provider_template.tokenizer_api_key,
    );
    update_yaml_field_if_needed(
        &mut file_value,
        "chat_endpoint",
        provider_dto.chat_endpoint,
        provider_template.chat_endpoint,
    );
    update_yaml_field_if_needed(
        &mut file_value,
        "completion_endpoint",
        provider_dto.completion_endpoint,
        provider_template.completion_endpoint,
    );
    update_yaml_field_if_needed(
        &mut file_value,
        "embedding_endpoint",
        provider_dto.embedding_endpoint,
        provider_template.embedding_endpoint,
    );
    update_yaml_field_if_needed(
        &mut file_value,
        "chat_default_model",
        provider_dto.chat_default_model,
        provider_template.defaults.chat_default_model,
    );
    update_yaml_field_if_needed(
        &mut file_value,
        "chat_light_model",
        provider_dto.chat_light_model,
        provider_template.defaults.chat_light_model,
    );
    update_yaml_field_if_needed(
        &mut file_value,
        "chat_thinking_model",
        provider_dto.chat_thinking_model,
        provider_template.defaults.chat_thinking_model,
    );
    file_value["enabled"] = serde_yaml::Value::Bool(provider_dto.enabled);

    let file_content = serde_yaml::to_string(&file_value).map_err(|e| {
        ScratchError::new(
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("Error parsing provider file: {}", e),
        )
    })?;
    tokio::fs::write(&provider_path, file_content.as_bytes())
        .await
        .map_err(|e| {
            ScratchError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                format!("Error writing provider file: {}", e),
            )
        })?;
...
pub async fn handle_v1_delete_provider(
    ...
    tokio::fs::remove_file(&provider_path).await.map_err(|e| {
        ScratchError::new(
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_v1_models(
    ...
    let provider = if use_server_provider {
        get_provider_from_server(gcx.clone())
            .await
            .map_err(|e| ScratchError::new(StatusCode::INTERNAL_SERVER_ERROR, e))?
    } else {
        let config_dir = gcx.read().await.config_dir.clone();
        get_provider_from_template_and_config_file(
            &config_dir,
            &params.provider_name,
            false,
            true,
...
pub async fn handle_v1_get_model(
    ...
    let model =
        match params.model_type {
            ModelType::Chat => {
                let model_name = params.model.ok_or_else(|| {
                    ScratchError::new(
                ...
                let chat_model =
                    provider
                        ...
                        .ok_or(ScratchError::new(
                            StatusCode::NOT_FOUND,
                            format!(
                                "Chat model {} not found for provider {}",
            ...
            ModelType::Completion => {
                if !provider.supports_completion {
                    return Err(ScratchError::new(
                        StatusCode::UNPROCESSABLE_ENTITY,
                        format!("Provider {} does not support completion", params.provider),
                    ));
                }
                let model_name = params.model.ok_or_else(|| {
                    ScratchError::new(
                ...
                let completion_model = provider.completion_models.get(&model_name).cloned().ok_or(
                    ScratchError::new(
                        StatusCode::NOT_FOUND,
                        format!(
                            "Completion model {} not found for provider {}",
                            model_name, params.provider
                        ),
                    ),
                )?;
...
pub async fn handle_v1_post_model(
    ...
    let post = serde_json::from_slice::<ModelPOST>(&body_bytes).map_err(|e| {
        ScratchError::new(
            StatusCode::UNPROCESSABLE_ENTITY,
    ...
    let provider_path = config_dir
        .join("providers.d")
        .join(format!("{}.yaml", post.provider));

    let _provider_template =
    ...
    fn get_or_create_model_mapping(
        file_value: &mut serde_yaml::Value,
        models_key: &str,
        model_name: &str,
    ) -> serde_yaml::Mapping {
    ...
    match post.model_type {
        ModelType::Chat => {
            let chat_model = serde_json::from_value::<ChatModelDTO>(post.model).map_err(|e| {
                ScratchError::new(
                    StatusCode::UNPROCESSABLE_ENTITY,
                    format!("Error parsing model: {}", e),
                )
            })?;
            let models_key = "chat_models";

            let mut model_value =
                get_or_create_model_mapping(&mut file_value, models_key, &chat_model.name);
            ...
            model_value.insert(
                "supports_multimodality".into(),
                chat_model.supports_multimodality.into(),
            );
            ...
            model_value.insert(
                "supports_boost_reasoning".into(),
                chat_model.supports_boost_reasoning.into(),
            );
            ...
            model_value.insert(
                ...
                match chat_model.default_temperature {
                    Some(default_temperature) => serde_yaml::Value::Number(
                        serde_yaml::Number::from(default_temperature as f64),
                    ),
                    None => serde_yaml::Value::Null,
                },
            );
        ...
        ModelType::Completion => {
            ...
            let mut model_value =
                get_or_create_model_mapping(&mut file_value, models_key, &completion_model.name);

            if let Some(model_family) = completion_model.model_family {
                let family_model_rec = get_known_models()
                    .completion_models
                    .get(&model_family.to_string())
                    .expect(&format!(
                        "Model family {} not found in known models",
                ...
                model_value.insert(
                    "scratchpad".into(),
                    family_model_rec.scratchpad.clone().into(),
                );
                model_value.insert(
                    "scratchpad_patch".into(),
                    serde_yaml::from_str(&family_model_rec.scratchpad_patch.to_string()).unwrap(),
                );
                model_value.insert(
                    "tokenizer".into(),
                    family_model_rec.base.tokenizer.clone().into(),
                );
            }
        ...
        ModelType::Embedding => {
            let embedding_model =
                serde_json::from_value::<EmbeddingModelDTO>(post.model).map_err(|e| {
                    ScratchError::new(
                        StatusCode::UNPROCESSABLE_ENTITY,
                        format!("Error parsing model: {}", e),
                    )
            ...
            model_value.insert(
                "embedding_size".into(),
                embedding_model.embedding_size.into(),
            );
            model_value.insert(
                "rejection_threshold".into(),
                serde_yaml::Value::Number(serde_yaml::Number::from(
                    embedding_model.rejection_threshold as f64,
                )),
            );
            model_value.insert(
                "embedding_batch".into(),
                embedding_model.embedding_batch.into(),
            );
...
pub async fn handle_v1_delete_model(
    ...
    let provider_path = config_dir
        .join("providers.d")
        .join(format!("{}.yaml", params.provider));
    ...
    match params.model_type {
        ModelType::Chat => {
            let model_name = params.model.as_ref().ok_or_else(|| {
                ScratchError::new(
                    StatusCode::BAD_REQUEST,
                    "Missing `model` query parameter".to_string(),
                )
            ...
            if !file_value.get(models_key).is_some()
                || !file_value[models_key].get(model_name).is_some()
            {
                return Err(ScratchError::new(
                    StatusCode::NOT_FOUND,
                    format!(
...
pub async fn handle_v1_model_default(
    ...
    let model_defaults = get_provider_model_default_settings_ui()
        .get(&params.provider)
        .ok_or_else(|| {
    ...
    let response_json = match params.model_type {
        ...
        ModelType::Completion => {
            serde_json::json!(CompletionModelDTO::new(model_defaults.completion.clone()))
        }
        ModelType::Embedding => {
            serde_json::json!(EmbeddingModelDTO::new(model_defaults.embedding.clone()))
        }
    };
