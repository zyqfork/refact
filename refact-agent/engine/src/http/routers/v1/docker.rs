...
pub async fn handle_v1_docker_container_action(
    ...
    let post = serde_json::from_slice::<DockerContainerActionPost>(&body_bytes).map_err(|e| {
        ScratchError::new(
    ...
    let (docker, _) = docker_and_isolation_load(gcx.clone()).await.map_err(|e| {
        ScratchError::new(
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("Cannot load docker tool: {}", e),
        )
    })?;
    ...
    let (output, _) = docker
        .command_execute(&docker_command, gcx.clone(), true, false)
        .await
        .map_err(|e| {
            ScratchError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                format!("Command {} failed: {}", docker_command, e),
            )
        })?;
    ...
    Ok(Response::builder()
        ...
        .body(Body::from(
            serde_json::to_string(&serde_json::json!({ "success": true, "output": output }))
                .unwrap(),
        ))
        .unwrap())
}
...
pub async fn handle_v1_docker_container_list(
    ...
    let post = serde_json::from_slice::<DockerContainerListPost>(&body_bytes).map_err(|e| {
        ScratchError::new(
            StatusCode::UNPROCESSABLE_ENTITY,
            format!("JSON problem: {}", e),
        )
    ...
    let docker_command = match post.label {
        Some(label) => {
            format!("container list --all --no-trunc --format json --filter label={label}")
        }
        None => "container list --all --no-trunc --format json".to_string(),
    };

    let unparsed_output = match docker
        .command_execute(&docker_command, gcx.clone(), true, false)
        .await
    {
    ...
    let mut output: Vec<Value> = unparsed_output
        .lines()
        ...
        .map_err(|e| {
            ScratchError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                format!("Container list JSON problem: {}", e),
            )
        })?;
    ...
    if let Some(image) = post.image {
        output = output
            ...
            .filter(|container| {
                container["Image"]
                    .as_str()
                    .map_or(false, |image_name| image_name.contains(&image))
            })
            .collect();
    }
        ...
        .map(|container| {
            container["ID"]
                ...
                .ok_or_else(|| {
                    ScratchError::new(
                        StatusCode::INTERNAL_SERVER_ERROR,
                        format!("Missing container ID in output:\n{:?}", output),
                    )
                })
        })
    ...
    let inspect_unparsed_output = match docker
        .command_execute(&inspect_command, gcx.clone(), true, false)
        .await
    {
    ...
    let inspect_output = serde_json::from_str::<Vec<serde_json::Value>>(&inspect_unparsed_output)
        ...
        ScratchError::new(
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("Container inspect JSON problem: {}", e),
        )
    })?;

    let containers: Vec<DockerContainerListOutput> = inspect_output
        .into_iter()
        .map(|container| {
            let mut container_name =
                extract_string_field(&container, &["Name"], "Missing container name")?;
            if container_name.starts_with('/') {
                container_name = container_name[1..].to_string()
            };
...
fn extract_string_field<'a>(
    ...
    field_path
        .iter()
        .fold(container, |acc, &key| &acc[key])
        .as_str()
        .map(ToString::to_string)
        .ok_or_else(|| {
            ScratchError::new(
