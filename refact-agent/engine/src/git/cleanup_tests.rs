...
async fn create_test_repository() -> (TempDir, HashMap<String, String>, HashMap<String, String>) {
    ...
    fs::write(&old_kept_file_path, "This file should be kept")
        .expect("Failed to write old kept file");

    let old_removed_file_path = repo_path.join("old_removed_file.txt");
    fs::write(&old_removed_file_path, "This file should be removed")
        .expect("Failed to write old removed file");

    let mut index = repo.index().expect("Failed to get index");
    index
        .add_path(std::path::Path::new("old_kept_file.txt"))
        .expect("Failed to add old kept file");
    index
        .add_path(std::path::Path::new("old_removed_file.txt"))
        .expect("Failed to add old removed file");
    index.write().expect("Failed to write index");

    let tree_id = index.write_tree().expect("Failed to write tree");
    let tree = repo.find_tree(tree_id).expect("Failed to find tree");

    let old_commit_oid = repo
        .commit(
            Some("HEAD"),
            &old_signature,
            &old_signature,
    ...
    let kept_entry = first_tree
        .get_name("old_kept_file.txt")
        .expect("Failed to find old_kept_file.txt in first commit");
    let removed_entry = first_tree
        .get_name("old_removed_file.txt")
        .expect("Failed to find old_removed_file.txt in first commit");

    blob_ids.insert("old_kept_blob".to_string(), kept_entry.id().to_string());
    blob_ids.insert(
        "old_removed_blob".to_string(),
        removed_entry.id().to_string(),
    );
    ...
    fs::write(&shared_file_path, "Shared content - version 1")
    ...
    index
        .remove_path(std::path::Path::new("old_removed_file.txt"))
        .expect("Failed to remove old_removed_file.txt");
    // Add the shared file
    index
        .add_path(std::path::Path::new("shared_file.txt"))
        .expect("Failed to add shared file");
    index
        .write()
        .expect("Failed to write index for second commit");

    let tree_id = index
        .write_tree()
        .expect("Failed to write tree for second commit");
    let tree = repo
        .find_tree(tree_id)
        .expect("Failed to find tree for second commit");
    let parent_commit = repo
        .find_commit(old_commit_oid)
        .expect("Failed to find parent commit");

    let old_commit2_oid = repo
        .commit(
            Some("HEAD"),
            &old_signature,
            &old_signature,
    ...
    fs::write(&shared_file_path, "Shared content - version 2 (recent)")
        .expect("Failed to update shared file");

    let recent_file_path = repo_path.join("recent_file.txt");
    fs::write(&recent_file_path, "This file should NOT be cleaned up")
        .expect("Failed to write recent file");

    index
        .add_path(std::path::Path::new("shared_file.txt"))
        .expect("Failed to add updated shared file");
    index
        .add_path(std::path::Path::new("recent_file.txt"))
    ...
    let tree = repo.find_tree(tree_id).expect("Failed to find tree");
    let parent_commit = repo
        .find_commit(old_commit2_oid)
        .expect("Failed to find parent commit");

    let recent_commit_oid = repo
        .commit(
            Some("HEAD"),
            &recent_signature,
            &recent_signature,
...
fn get_all_objects(repo_path: &std::path::Path) -> Result<HashSet<String>, String> {
    ...
    for entry in
        fs::read_dir(&objects_dir).map_err(|e| format!("Failed to read objects dir: {}", e))?
    {
        ...
        if path.is_dir() && path.file_name().unwrap().to_str().unwrap().len() == 2 {
            ...
            for obj_entry in
                fs::read_dir(&path).map_err(|e| format!("Failed to read object subdir: {}", e))?
            {
                let obj_entry =
                    obj_entry.map_err(|e| format!("Failed to read object entry: {}", e))?;
...
fn verify_blob_exists_with_content(
    repo: &Repository,
    objects_after: &HashSet<String>,
    blob_id: &str,
    expected_content: &str,
    description: &str,
) {
    assert!(
        objects_after.contains(blob_id),
        "{} blob should exist in object store",
        description
    );
    let oid = git2::Oid::from_str(blob_id).expect("Invalid blob ID");
    let blob = repo
        .find_blob(oid)
        .expect(&format!("Failed to find {} blob", description));
    let content = std::str::from_utf8(blob.content())
        .expect(&format!("Failed to read {} content", description));
    assert_eq!(
...
fn verify_blob_removed(objects_after: &HashSet<String>, blob_id: &str, description: &str) {
    assert!(
        !objects_after.contains(blob_id),
        "{} blob should have been cleaned up",
        description
    );
    println!("✓ {} blob was successfully cleaned up", description);
}

fn verify_file_in_head_with_content(
    repo: &Repository,
    objects_after: &HashSet<String>,
    filename: &str,
    expected_content: &str,
    ...
    let entry = head_tree
        .get_name(filename)
        .expect(&format!("{} should be in HEAD", filename));
    let blob = repo
        .find_blob(entry.id())
        .expect(&format!("Failed to find {} blob", filename));
    let content =
        std::str::from_utf8(blob.content()).expect(&format!("Failed to read {} content", filename));
    assert_eq!(content, expected_content, "{} has wrong content", filename);
    assert!(
        objects_after.contains(&entry.id().to_string()),
        "{} blob should exist in object store",
        filename
    );
...
async fn test_cleanup_old_objects_comprehensive() {
    ...
    let should_be_removed: HashSet<String> = all_old_objects
        .difference(&recent_objects)
        .cloned()
        .collect();
    ...
    let removed_count =
        cleanup::cleanup_old_objects_from_single_repo(repo_path, Arc::new(AtomicBool::new(false)))
            .await
    ...
    verify_blob_removed(
        &objects_after,
        &blob_ids["old_removed_blob"],
        "old_removed_file.txt",
    );
    verify_blob_exists_with_content(
        &repo,
        &objects_after,
        &blob_ids["old_kept_blob"],
        "This file should be kept",
        "old_kept_file.txt",
    );

    verify_file_in_head_with_content(
        &repo,
        &objects_after,
        "old_kept_file.txt",
        "This file should be kept",
    ...
    verify_file_in_head_with_content(
        ...
        &objects_after,
        "shared_file.txt",
        "Shared content - version 2 (recent)",
    );
    verify_file_in_head_with_content(
        &repo,
        &objects_after,
        "recent_file.txt",
        "This file should NOT be cleaned up",
    );

    let missing_objects: HashSet<String> =
        should_be_kept.difference(&objects_after).cloned().collect();
    assert!(
        missing_objects.is_empty(),
        "Expected objects missing after cleanup: {:?}",
        missing_objects
    );
    println!("✓ All expected objects preserved");

    let removed_objects: HashSet<String> =
        objects_before.difference(&objects_after).cloned().collect();
    ...
    index
        .add_path(std::path::Path::new("new_file_after_cleanup.txt"))
        .expect("Failed to add new file to index");
    index.write().expect("Failed to write index after cleanup");

    let tree_id = index
        .write_tree()
        .expect("Failed to write tree after cleanup");
    let tree = repo
        .find_tree(tree_id)
        .expect("Failed to find tree after cleanup");
    ...
    println!(
        "✓ Successfully created new commit after cleanup: {}",
        new_commit_oid
    );
    ...
    let head_commit = repo
        ...
        .unwrap()
        .peel_to_commit()
        .expect("Failed to get HEAD after new commit");
    println!(
        "✓ Repository HEAD accessible: {}",
