    ...
    loop {
        ...
        let (cache_dir, abort_flag) = {
            ...
            (
                ...
                gcx_locked.git_operations_abort_flag.clone(),
            )
        };
        ...
        match cleanup_old_objects_from_repos(
            &cache_dir.join("shadow_git"),
        ...
        .await
        {
            Ok(objects_cleaned) => {
                if objects_cleaned > 0 {
                    tracing::info!(
                        "Git object cleanup: removed {} old objects from active repositories",
                        objects_cleaned
                    );
                }
            }
...
async fn cleanup_old_objects_from_repos(
    dir: &Path,
    workspace_folder_hashes: &[String],
        ...
        .map_err(|e| format!("Failed to read directory entry: {}", e))?
    {
        ...
        match cleanup_old_objects_from_single_repo(&path, abort_flag.clone()).await {
            Ok(removed_count) => {
                if removed_count > 0 {
                    tracing::info!(
                        "Cleaned {} old objects from repository: {}",
                        removed_count,
                        path.display()
                    );
            ...
            Err(e) => {
                tracing::warn!(
                    "Failed to cleanup objects from repository {}: {}",
                    path.display(),
                    e
                );
            }
...
pub async fn cleanup_old_objects_from_single_repo(
    repo_path: &Path,
    abort_flag: Arc<AtomicBool>,
) -> Result<usize, String> {
    ...
    let (recent_objects, old_objects) =
        collect_objects_from_commits(&repo, cutoff_time, abort_flag)?;
...
fn collect_objects_from_commits(
    ...
    for oid_result in revwalk {
        ...
        let commit_time =
            SystemTime::UNIX_EPOCH + std::time::Duration::from_secs(commit.time().seconds() as u64);
        let is_recent = commit_time >= cutoff_time || Some(oid) == head_oid;

        let tree_oid = commit.tree_id();
        let objects_set = if is_recent {
            &mut recent_objects
        } else {
            &mut old_objects
        };
    ...
    for object_id in objects_to_remove {
        ...
        if object_path.exists() {
            match tokio::fs::remove_file(&object_path).await {
                Ok(()) => removed_count += 1,
                Err(e) => tracing::warn!(
                    "Failed to remove blob object file {}: {}",
                    object_path.display(),
                    e
                ),
            }
