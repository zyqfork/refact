...
pub async fn forward_remote_docker_if_needed(
    docker_daemon_address: &str,
    ...
    let remote_port_or_socket = match docker_daemon_address {
        ...
        _ if docker_daemon_address.starts_with("unix://")
            || docker_daemon_address.starts_with("npipe://") =>
        {
    ...
    let ssh_tunnel = ssh_tunnel_open(
        &mut vec![Port {
            published: "0".to_string(),
            target: remote_port_or_socket,
        }],
        ssh_config,
    )
    .await?;
...
pub async fn ssh_tunnel_check_status(ssh_tunnel: &mut SshTunnel) -> Result<(), String> {
    ...
    if let Some(status) = exit_status {
        return Err(format!(
            "SSH tunnel process exited with status: {:?}",
            status
        ));
    }

    let (_, stderr_output, _) = blocking_read_until_token_or_timeout(
        &mut ssh_tunnel.stdout,
        &mut ssh_tunnel.stderr,
        100,
        "",
    )
    .await?;
...
pub async fn ssh_tunnel_open(
    ...
    let mut stdout = process
        .stdout
        .take()
        .ok_or("Failed to open stdout for ssh process")?;
    let mut stderr = process
        .stderr
        .take()
        .ok_or("Failed to open stderr for ssh process")?;

    let (_, output_stderr, _) =
        blocking_read_until_token_or_timeout(&mut stdout, &mut stderr, 100, "").await?;
    if !output_stderr.is_empty() {
        return Err(format!("SSH error: {}", output_stderr));
    }

    let port_to_test_connection = ports_to_forward
        .iter()
        .next()
        .ok_or_else(|| "Failed to get port to test connection".to_string())?;
    for attempt in 0..25 {
        match TcpStream::connect(format!("127.0.0.1:{}", &port_to_test_connection.published)).await
        {
            Ok(_) => {
                info!(
                    "huzzah, it worked: connect to 127.0.0.1:{}",
                    port_to_test_connection.published
                );
                return Ok(SshTunnel {
            ...
            Err(e) => {
                info!(
                    "this should eventually work: connect to 127.0.0.1:{} attempt {}: {}",
                    port_to_test_connection.published,
                    attempt + 1,
                    e
                );
                let (_, stderr_output, _) =
                    blocking_read_until_token_or_timeout(&mut stdout, &mut stderr, 400, "").await?;
                if !stderr_output.is_empty() {
    ...
    return Err(format!(
        "Failed to connect to 127.0.0.1:{}, max attempts reached",
        &port_to_test_connection.published
    ));
}
