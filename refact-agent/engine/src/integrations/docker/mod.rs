...
pub async fn docker_and_isolation_load(
...
) -> Result<(ToolDocker, Option<SettingsIsolation>), String> {
    let include_paths_matching = [
        "**/docker.yaml".to_string(),
        "**/isolation.yaml".to_string(),
    ];
    let (integrations, _yaml_errors) =
        load_integrations(gcx.clone(), &include_paths_matching).await;

    let docker_tools = integrations
        .get("docker")
    ...
    let docker_tool = docker_tools[0]
        .as_any()
        .downcast_ref::<ToolDocker>()
        .ok_or("Failed to downcast docker tool".to_string())?
        .clone();

    let isolation_integration = integrations
        .get("isolation")
